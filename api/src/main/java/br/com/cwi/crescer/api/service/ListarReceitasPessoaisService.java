...
@Service
public class ListarReceitasPessoaisService {
    @Autowired
    ...
    public Page<ReceitaResponse> listar(Pageable pageable) {
        Usuario usuario = usuarioAutenticadoService.get();
        Page<Receita> receitas = receitaRepository.findByUsuario_Id(usuario.getId(), pageable);

        return receitas.map(this::transformar);
    }

    public ReceitaResponse transformar(Receita receita) {
